...
class AnomalyDetectionUI:
    ...
    def scan_for_graphs(self, script_name):
        ...
        graph_patterns = {
            ...
            "IsolationForest.py": ["isolation_forest_results.png", "if_anomalies.png"],
            "DBSCAN.py": ["dbscan_results.png", "dbscan_clusters.png"],
            "hybrid_integration.py": ["hybrid_results.png", "anomaly_comparison.png"],
            "sensor_visualization.py": ["sensor_viz.png", "sensor_patterns.png"],
            "3d_visualization.py": ["3d_anomalies.png", "3d_scatter.png"],
            "feature_contribution.py": ["feature_importance.png", "shap_values.png"],
        }
        
        # Also scan for any PNG files in current directory
        if script_name in graph_patterns:
            for pattern in graph_patterns[script_name]:
                if os.path.exists(pattern):
                    ...
                    self.add_graph_tab(pattern, tab_name)
        ...
        try:
            ...
            # Add up to 3 most recent PNG files
            for png_file in png_files[:3]:
                if png_file not in [os.path.basename(p) for p in self.graph_paths]:
                    tab_name = png_file.replace('.png', '').replace('_', ' ').title()
                    self.add_graph_tab(png_file, tab_name)
        except Exception as e:
            pass
